...
**Status:** feature-complete. Compiled 2026-06-17; updated 2026-06-18. **Phases 0–5 complete** (headless engine ships rectangular / chevron / interdigitated **sliders and wheels** plus a two-layer mutual-cap **XY diamond trackpad** with B.Cu via bridges, all DRC-clean in KiCad 10; a PySide6 GUI with a slider/wheel/trackpad switcher gives a live preview and export). Phase 5 adds **vendor-pinned presets** (verified against the guideline tables), **fab-rule guards** (`default`/`jlcpcb`/`oshpark` profiles; warn by default, `--strict` blocks; live in the GUI), a **standalone PyInstaller binary** (Linux build + headless smoke test verified locally; macOS/Windows via CI matrix), and a **[usage guide](./usage.md)**.
...
| **5 — Polish & distribution** ✅ | Vendor presets, fab-min-rule guards, cross-platform binaries, user docs. | Installable binary; presets reproduce vendor reference dimensions. Done: a `params/fab.py` layer with `default`/`jlcpcb`/`oshpark` profiles deriving the tightest copper/clearance/drill/annular dimensions per widget (warn by default, CLI `--strict` blocks with exit 3, live amber banner in the GUI); presets pinned to their cited vendor tables by test; a PyInstaller spec freezing the CLI+GUI into one binary (`captouch gui` launches the app) — Linux binary built and headlessly smoke-tested (`--version`, generation, `--strict` gate, offscreen GUI), macOS/Windows built + smoke-tested by the `build-binaries` CI matrix; a `docs/usage.md` user guide. |
