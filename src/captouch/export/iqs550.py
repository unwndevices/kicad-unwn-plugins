...
def active_channel_bytes(
    geo: TrackpadGeometry, threshold: float = DISABLE_AREA_FRACTION
) -> bytes:
...
def render_iqs550_config(
    ...
    shape = (
        f"{p.mask_shape} mask / {p.clip_mode} clip"
        if p.mask_shape != "rect"
        else "rectangular"
    )
    lines: list[str] = []
    lines.append("/*")
    ...
    lines.append(" *")
    lines.append(
        f" * Sensor: {r} Rx × {c} Tx diamond matrix ({p.num_nodes} nodes), {shape}."
    )
    lines.append(
    ...
    lines.append(" *")
    lines.append(
        " * Write these to the device after power-up (before AUTO_ATI), per AZD070. The"
    )
    lines.append(
        " * Rx/Tx MAPPING registers (0x063F / 0x0649) are left at their defaults; this"
    )
    lines.append(
        " * bitmap assumes the identity mapping (rows→Rx0.., cols→Tx0..). If you route"
    )
    ...
    lines.append("")
    lines.append(f"#define IQS550_TOTAL_RX           {r}      /* reg 0x{TOTAL_RX_ADDR:04X} (§5.1.1) */")
    lines.append(f"#define IQS550_TOTAL_TX           {c}      /* reg 0x{TOTAL_TX_ADDR:04X} (§5.1.1) */")
